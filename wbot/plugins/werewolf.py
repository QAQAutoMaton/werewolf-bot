...
async def set(session: CommandSession):
    ...
    if group_id in game:
        ...
        game[group_id]=Game()
    if not 'type' in session.state:
        await send_at(session,"""用法：#set 配置 位置
如： #set pwbynls 0
...
""")
    ...
    # 从会话状态（session.state）中获取城市名称（city），如果当前不存在，则询问用户
        ...
        if not (0<=at and at<=len(type)):
            await send_at(session,"位置为一个[0..人数]之间的整数")
            return
        

        game[group_id].init(s,len(type))
...
@set.args_parser
async def set_parser(session:CommandSession):
...
async def kick(session: CommandSession):
    ...
    if not group_id:
        await session.send('请在群聊中使用狼人杀功能')
        return
    if not user_id in werewolf_admin:
